function [Q0_new, multi_Q0, M0, src_vox, frame_err, frame_idx, mesh_errs_t]=FindBestQ0(sigmab, Bh, heart_pts, sub_vox, elec_pts, model_info, Vd, method, fwd_map, construct_map, num_src, file_ext)
...
%                         line_length=fprintf("Percentage of Frames Simulated:  %.4g %%",100*(j/numHpts));
            ...
            [err_val, minCC_idx]=mink(cycle_err, num_src);
        ...
        V_sim=zeros(L,num_data_volt);
        for t=1:num_data_volt
            [xt, ~]=RegSystem(Gsim, Vd(:,t), 'none', []);
            V_sim(:,t)=Gsim*xt;

        end

        Recon_Error=CC_RelativeError(V_sim, Vd)
        
        
        
        %% Gets Multi Src Recon Error
        Gsim=zeros(L, 3*num_src);
        for k=1:num_src
            Gk=G(:,:,minCC_idx(k));
            Gsim(:,3*(k-1)+1:3*k)=Gk;
            ...
            MS_err(k)=CC_RelativeError(V_sim, Vd);
